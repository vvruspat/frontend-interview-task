...
let jwtToken: string | null = localStorage.getItem('jwt') ? (JSON.parse(localStorage.getItem('jwt') || '') as TLoginJWT)?.token ?? null : null;
...
export const getReviews = (limit: number = 20, offset: number = 0, theme_id?: number): Promise<ApiResponse<{data: TFeedbackItem[]}>> => {
...
export const getReview = (id: TFeedbackItem['id']): Promise<ApiResponse<TFeedbackItem>> => {
  return http(`${apiUrl}/api/reviews?id=${id}`);
}

// In swagger there is no data in type, but in reality it exists
export const getThemes = (limit: number = 100, offset: number = 0): Promise<ApiResponse<{data: TTheme[]}>> => {
  const params: Record<string, string | number> = {
    limit,
    offset,
  };
...
export const getTheme = (id: TTheme['id']): Promise<ApiResponse<TTheme>> => {
  return http(`${apiUrl}/api/themes?id=${id}`);
}

export const auth = (username: string, password: string): Promise<ApiResponse<TLoginJWT>> => {
